...
download.
  ...
  var.private_key_file == "" ? module.ssh_keypair_aws.private_key_filename : var.private_key_file,
  var.private_key_file == "" ? module.ssh_keypair_aws.private_key_filename : var.private_key_file,
  )}
...
output "vpc_cidr" {
  value = var.create_vpc ? element(concat(aws_vpc.main.*.cidr_block, [""]), 0) : var.vpc_cidr # TODO: Workaround for issue #11210
}

output "vpc_id" {
  value = var.create_vpc ? element(concat(aws_vpc.main.*.id, [""]), 0) : var.vpc_id # TODO: Workaround for issue #11210
}
